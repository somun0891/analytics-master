...
def test_manifest_reader_token(manifest_file):
    """
    Test manifest file reader and look for token value and streaming
    """
    for _, spec in manifest_file.items():
        assert spec.get("streaming") in [True, False]
        assert spec.get("private_token") in [
            None,
