...
{{ simple_cte([
    ...
    ('zuora_rate_plan', 'zuora_central_sandbox_rate_plan_source'),
    ('zuora_rate_plan_charge', 'zuora_central_sandbox_rate_plan_charge_source'),
    ...
    ('zuora_charge_metrics', 'zuora_central_sandbox_charge_metrics_source')
])}}
...
, sfdc_account AS (
...
), latest_zuora_charge_metrics AS (

    SELECT *
    FROM zuora_charge_metrics
    QUALIFY ROW_NUMBER() OVER (PARTITION BY rate_plan_charge_id ORDER BY start_date DESC) = 1
    ...
    SELECT
      ...
      zuora_rate_plan.amendment_type                                    AS rate_plan_charge_amendment_type,
      ...
      latest_zuora_charge_metrics.mrr_gross_amount                             AS zuora_charge_metrics_gross_amount_mrr,
      latest_zuora_charge_metrics.mrr_net_amount                               AS zuora_charge_metrics_net_amount_mrr,
      latest_zuora_charge_metrics.mrr_discount_amount                          AS zuora_charge_metrics_discount_amount_mrr,
      latest_zuora_charge_metrics.tcv_gross_amount                             AS zuora_charge_metrics_gross_amount_tcv,
      latest_zuora_charge_metrics.tcv_net_amount                               AS zuora_charge_metrics_net_amount_tcv,
      latest_zuora_charge_metrics.tcv_discount_amount                          AS zuora_charge_metrics_discount_amount_tcv,
    ...
    INNER JOIN zuora_rate_plan_charge
      ON zuora_rate_plan.rate_plan_id = zuora_rate_plan_charge.rate_plan_id
    LEFT JOIN latest_zuora_charge_metrics
      ON zuora_rate_plan_charge.rate_plan_charge_id = latest_zuora_charge_metrics.rate_plan_charge_id
    ...
    SELECT
      ...
      NULL                                                                                  AS zuora_charge_metrics_gross_amount_mrr,
      NULL                                                                                  AS zuora_charge_metrics_net_amount_mrr,
      NULL                                                                                  AS zuora_charge_metrics_discount_amount_mrr,
      NULL                                                                                  AS zuora_charge_metrics_gross_amount_tcv,
      NULL                                                                                  AS zuora_charge_metrics_net_amount_tcv,
      NULL                                                                                  AS zuora_charge_metrics_discount_amount_tcv,
