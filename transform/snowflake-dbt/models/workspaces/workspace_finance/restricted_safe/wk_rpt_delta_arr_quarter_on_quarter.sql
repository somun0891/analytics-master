...
py_arr_with_cy_parent AS (
  ...
  SELECT
    ...
    DATEADD('month', 3, child_account_arrs.arr_month)        AS delta_arr_month,
    dim_crm_account_daily_snapshot.dim_parent_crm_account_id AS parent_account_id_in_delta_arr_month,
    DATEADD('month', 3, dim_date.snapshot_date_fpa)          AS retention_period_snapshot_date,
  ...
  LEFT JOIN dim_crm_account_daily_snapshot
    ON child_account_arrs.child_account_id = dim_crm_account_daily_snapshot.dim_crm_account_id
      AND dim_crm_account_daily_snapshot.snapshot_date = DATEADD('month', 3, dim_date.snapshot_date_fpa)
  ...
  SELECT
    ...
    DATEADD('month', 3, child_account_arrs.arr_month)        AS delta_arr_month,
    dim_crm_account_daily_snapshot.dim_parent_crm_account_id AS parent_account_id_in_delta_arr_month,
    DATEADD('month', 3, dim_date.snapshot_date_fpa_fifth)    AS retention_period_snapshot_date,
  ...
  LEFT JOIN dim_crm_account_daily_snapshot
    ON child_account_arrs.child_account_id = dim_crm_account_daily_snapshot.dim_crm_account_id
      AND dim_crm_account_daily_snapshot.snapshot_date = DATEADD('month', 3, dim_date.snapshot_date_fpa_fifth)
...
final_arr AS (
  ...
  SELECT
    ...
    cy_arr_with_cy_parent.retained_product_category                                                                                                                                            AS current_quarter_product_category,
    py_arr_with_cy_parent.py_product_category                                                                                                                                                  AS prior_quarter_product_category,
    cy_arr_with_cy_parent.retained_product_ranking                                                                                                                                             AS current_quarter_product_ranking,
    py_arr_with_cy_parent.py_product_ranking                                                                                                                                                   AS prior_quarter_product_ranking,
    CASE
    ...
    END                                                                                                                                                                                        AS prior_quarter_arr_band,
    SUM(ZEROIFNULL(py_arr_with_cy_parent.py_mrr))                                                                                                                                              AS prior_quarter_mrr,
    SUM(ZEROIFNULL(cy_arr_with_cy_parent.retained_mrr))                                                                                                                                        AS current_quarter_mrr,
    SUM(ZEROIFNULL(py_arr_with_cy_parent.py_arr))                                                                                                                                              AS prior_quarter_arr,
    SUM(ZEROIFNULL(cy_arr_with_cy_parent.retained_arr))                                                                                                                                        AS current_quarter_arr,
    CASE
...
final_with_delta AS (
  SELECT
    ...
    CASE
      ...
      WHEN current_quarter_arr = 0
        AND ABS(prior_quarter_arr) > 0 THEN 'Churn'
      WHEN current_quarter_arr < prior_quarter_arr
        AND current_quarter_arr > 0 THEN 'Contraction'
      WHEN current_quarter_arr > prior_quarter_arr THEN 'Expansion'
      WHEN current_quarter_arr = prior_quarter_arr THEN 'No Impact'
    ...
    CASE
      WHEN
        ...
        THEN ZEROIFNULL(
            prior_quarter_arr / NULLIF(prior_year_quantity, 0) * (net_retention_quantity - prior_year_quantity)
          )
      WHEN prior_year_quantity != net_retention_quantity
        AND prior_year_quantity = 0 THEN current_quarter_arr
      ELSE 0
    END AS seat_change_arr,
    CASE
      WHEN
        is_arpu_combined = 'N'
        AND type_of_arr_change IN ('Expansion', 'Contraction') THEN COALESCE(current_quarter_arr, 0) - COALESCE(prior_quarter_arr, 0)
      WHEN
        is_arpu_combined = 'Y'
        AND prior_quarter_product_category = current_quarter_product_category
        THEN net_retention_quantity * (
            current_quarter_arr / NULLIF(net_retention_quantity, 0) - prior_quarter_arr / NULLIF(prior_year_quantity, 0)
          )
      WHEN
        is_arpu_combined = 'Y'
        AND prior_quarter_product_category != current_quarter_product_category
        AND prior_quarter_product_ranking = current_quarter_product_ranking
        THEN net_retention_quantity * (
            current_quarter_arr / NULLIF(net_retention_quantity, 0) - prior_quarter_arr / NULLIF(prior_year_quantity, 0)
          )
      ELSE 0
    ...
    (
      CASE
        WHEN
          ...
          THEN ZEROIFNULL(
              net_retention_quantity * (
                current_quarter_arr / NULLIF(net_retention_quantity, 0) - prior_year_quantity / NULLIF(prior_year_quantity, 0)
              )
            )
        ELSE 0
      END
